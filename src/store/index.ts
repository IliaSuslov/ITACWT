...
export const useAppStore = create<Store>((set) => ({
    ...
    filteredData: [],
    filters: { search: '', status: 'all', dataset: '' },
    actions: {
        update: (dataSetKey, id, updated) => set((state) => {
            if (!(dataSetKey in state.data)) {
                console.error(`Invalid dataset key: ${dataSetKey}`);
                return state;
            }
            const currentDataset = state.data[dataSetKey];
            const updatedData = currentDataset.map((item) => {
                if (item.id === id) {
                    return { ...item, ...updated };
                }
                return item;
            ...
            if (!currentDataset.some(item => item.id === id)) {
                console.warn(`Item with ID ${id} not found in ${dataSetKey}`);
            }
            ...
            return {
                ...
                data: {
                    ...state.data,
                    [dataSetKey]: updatedData
                }
            };
        }),
        applyFilters: (filters) =>
            set((state) => {
                const { search, status = 'all', dataset } = filters;
                const searchTerm = search?.toLowerCase() ?? '';

                if (!(dataset in state.data) || dataset === '') {
                    console.error(`Invalid dataset: ${dataset}`);
                    return state;
                }

                const currentData = state.data[dataset];

                const filtered = currentData.filter((item: { active?: boolean;[key: string]: unknown }) => {
